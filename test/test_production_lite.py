#!/usr/bin/env python3
"""
LTX-2 生产级测试脚本 (Lite版本)
基于 workflow_api_format.json，移除可选的 MelBandRoformer 节点
保留核心功能：LoRA、图片预处理、完整生成流程
"""
import json
import time
import random
...
def create_workflow(image_name, audio_name, seed=None):
    """
    创建完整工作流（去掉 MelBandRoformer）
    直接使用原始音频，不做人声分离
    """
    if seed is None:
    ...
    return {
        ...
        # 音频 VAE 编码（直接使用裁剪后的音频，不做人声分离）
        "242": {
            "inputs": {
                "audio": ["244", 0],  # 改为直接使用裁剪后的音频
                "audio_vae": ["171", 0]
            },
...
def main():
    print("=" * 70)
    print("LTX-2 Production Test - Lite Version")
    print("Full workflow without MelBandRoformer (vocal separation)")
    print("=" * 70)
    ...
    try:
        with request.urlopen(req, timeout=30) as resp:
            ...
            print("   • Audio: Direct encoding (full audio)")
