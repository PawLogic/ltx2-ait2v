#!/usr/bin/env python3
"""
LTX-2 完整测试 - 基于 LTX2-I2V-Audio.json 参考工作流
使用正确的节点配置
"""
import json
import time
...
def main():
    print("=" * 70)
    print("LTX-2 完整功能测试 - 参考工作流版本")
    print("基于 LTX2-I2V-Audio.json")
    print("=" * 70)
    ...
    print("   🎵 音频: 10秒完整")
    print("   🖼️  图片: 768x512, img_compression=42 (参考值)")
    print("   🎬 输出: 297帧 @ 30fps = 9.9秒 (精确同步)")
    print("   ⚙️  采样: 8步 + CFG 1.0 (参考配置)")
    ...
    workflow = {
        ...
        # 2. LTXAVTextEncoderLoader - 加载文本编码器（关键！）
        "155": {
            "inputs": {
        ...
        # 18-19. 文本编码 (使用 LTXAVTextEncoderLoader 的输出)
        "169": {
            "inputs": {
                "text": "A person speaks naturally with perfect lip synchronization",
                "clip": ["155", 0]  # 从 LTXAVTextEncoderLoader 获取
            },
    ...
    print("\n🚀 步骤 3/3: 提交工作流...")
    payload = {"prompt": workflow, "client_id": f"ref_test_{int(time.time())}"}
    ...
    try:
        with request.urlopen(req, timeout=30) as resp:
            ...
            print("=" * 70)
            print(f"\n⏱️  预计: ~3-5分钟")
            print(f"📁 输出: /workspace/ComfyUI/output/LTX2_Final_*.mp4")
            print("\n" + "=" * 70)
